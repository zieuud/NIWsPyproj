...
for i in range(9, 13):
    ...
    t_an[i-1-8] = temp.variables['t_an'][0, :, lat_idx, lon_idx]
    salt = nc.Dataset(path2)
    s_an[i-1-8] = salt.variables['s_an'][0, :, lat_idx, lon_idx]
for i in range(1, 9):
    path1 = r'K:\grad_proj\WOA23\woa23_B5C2_t{:02}_04.nc'.format(i)
    path2 = r'K:\grad_proj\WOA23\woa23_B5C2_s{:02}_04.nc'.format(i)
    temp = nc.Dataset(path1)
    t_an[i-1+4] = temp.variables['t_an'][0, :, lat_idx, lon_idx]
    salt = nc.Dataset(path2)
    s_an[i-1+4] = salt.variables['s_an'][0, :, lat_idx, lon_idx]
