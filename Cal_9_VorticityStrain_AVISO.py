...
# ---------- for my mooring ----------
aviso = nc.Dataset(r'L:\graduation proj\data\AVISO\cmems_obs-sl_glo_phy-ssh_my_allsat-l4-duacs-0.125deg_P1D_20150801-20160731_-31--33_35-37.nc')
...
# for my mooring
dys = [R * np.cos(np.radians(i)) * np.radians(0.125) for i in [36.1875, 36.1875, 36.3125, 36.3125]]
selections = [[-32.6875, 36.1875], [-32.8125, 36.1875], [-32.6875, 36.3125], [-32.8125, 36.3125]]
vorticitySelections = {}
strainSelections = {}
...
for vorticity in vorticitySelections.values():
    path = r'ReanaData\AVISO_0125_vorticity{}.npy'.format(num)
    np.save(path, vorticity)
    num += 1
num = 1
for strain in strainSelections.values():
    path = r'ReanaData\AVISO_0125_strain{}.npy'.format(num)
    np.save(path, strain)
    num += 1
